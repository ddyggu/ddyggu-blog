...
jQuery.fn.wowSlider=function(E){
	...
	function b(c) { 
		...
		M("<div>").css({
			...
			visibility:"hidden",
			"font-size":0,
			"line-height":0}).append(
					a.find("li:first img:first").clone().css({width:"100%"})).prependTo(a);
		R.css({
			position:"absolute",
			top:0,
			animation:"none",
		...
		if((/MSIE/.test(z)&&parseInt(/MSIE\s+([\d\.]+)/.exec(z)[1],10)<8)||(/Safari/.test(z))){
			...
			R.css({width:aa+"00%"});
			j.css({width:100/aa+"%"})}
		else{R.css({width:I+"00%",display:"table"});
			j.css({display:"table-cell","float":"none",width:"auto"})}
		
		var G=E.onBeforeStep||function(c){return c+1};
		E.startSlide=D(isNaN(E.startSlide)?G(-1,I):E.startSlide);b(E.startSlide);
		...
		function k(al,f,c){
			if(isNaN(al)){al=G(H,I)}al=D(al);if(H==al){
				return
			}
			if(t){t.load(al,function(){
				u(al,f,c)})}
			else{u(al,f,c)}}
		...
		if(!d) { 
			return 
		} else{ab(O?O.parentNode:a.get(0),
				function(al,f,c){
					...
					function() {
						...
						if(c){ var f=document.createEvent("HTMLEvents");
		...
		function K(c){
			...
			if(E.autoPlay){
				p=setTimeout(function(){
					if(!J){ k() }}, E.delay+(c?0:E.duration));ad()
			} else{ 
				x()
			}}
		...
		var S=ak||document.body;d=d.replace(/^\s+|\s+$/g,"");
		ak=d?M("<div>"):0;
		
		M(ak).css({
			position:"absolute",
			padding:"0 0 0 0"}).appendTo(S);
			...
			if(V){ 
				V.css({
					...
					href:"http://"+d.toLowerCase()
						...
						function(c){
							return false
				}).hide().appendTo(ak||document.body).attr("target","_blank")
			}
				...
				if(n.length){ 
					...
					if(am.length){
						...
						function ar(aB){
							...
							an.show();
							var aC={ left : av.get(aB).offsetLeft-an.width()/2,
		...
		if(E.caption){
			...
			l.append($caption);
			$caption.bind("mouseover",function(c){q()});
			$caption.bind("mouseout",function(c){K()})}
			...
			var U={ none:function(f,c){c.show()},
					  fade:function(al,c,f){c.fadeIn(f,C)},
					  array:function(al,c,f){o(c,al[Math.floor(Math.random()*al.length)],0.5,"easeOutElastic1",f)},
					  move:function(al,c,f){ U.array([ {left1:"100%",top2:"100%"},
					                                   		{left1:"80%",left2:"-50%"},
					  ...
					  slide:function(al,c,f){ Y(c,
							  {direction:"left",easing:"easeInOutExpo",complete:function(){
								  if(c.get(0).filters){ c.get(0).style.removeAttribute("filter")}} ,duration:f})}
					  };
			...
			function F(f){
				...
				if(!am.replace(/\s+/g,"")) { am="" }
				var c=M(".ws-title",l);
				c.stop(1,1).stop(1,1).fadeOut(E.captionDuration/3,function(){ 
					if(am||al){c.html((am?"<span>"+am+"</span>":"")+(al?"<div>"+al+"</div>":""));
			...
			function Q(an,f){ 
				...
				if(al&&al.getComputedStyle){ 
					...
					if(am){ ao=am.getPropertyValue(f)} 
					} else {
			...
			function w(am,al,ap){
				...
				if(al){c+=an}
				if(ap){c+=(parseFloat(Q(am,"margin-top"))||0)+(parseFloat(Q(am,"margin-bottom"))||0)}
				return c
			}
			...
			function o(an,ar,c,ap,al) {
				...
				function ao(au){ 
					...
					M(am[au]).css({visibility:"visible"})
								.css({opacity:0})
								.animate({opacity:av},al,"easeOutCirc")
								.animate({top:0,left:0} , {duration:al,easing:(ar.easing||ap),queue:false})
				}
				
				ao(0);
				
				setTimeout(function(){
					ao(1)},al*(ar.distance||c))
			}
			...
			function Y(aq,au){
				...
				for(var al in at){
					at[al]=aq.get(0).style[al]}aq.show();
					var ap={ width:B(aq.get(0),1,1),
					...
					if(aq.css("position")=="static"){
						...
						M.extend(ap,{position:aq.css("position"),
									     zIndex:aq.css("z-index")});
										 aq.css({position:"absolute",
										 top:0,
										 left:0,
										 right:"auto",
										 bottom:"auto"})
					...
					aq.animate(ao, {queue:false, duration:au.duration, easing:au.easing,
										complete:function(){
											aq.css(at);
					...
					if(E.autoPlay){
						...
						swfobject.createSWF({
							...
							{allowScriptAccess:"always",
							 loop:true,
							 FlashVars:"listener="+s+"&loop=1&autoplay="+(E.autoPlay?1:0)+"&mp3="+L},m);
						L=0
					}
